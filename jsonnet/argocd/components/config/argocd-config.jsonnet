...
function(params) {
  ...
  argocdConfig: holder {
    ...
    config_map_argocd_gpg_keys_cm: std.map(kustomization, [holder.config_map_argocd_gpg_keys_cm])[0],
    config_map_argocd_rbac_cm: std.map(kustomization, [holder.config_map_argocd_rbac_cm])[0],
    ...
    config_map_argocd_tls_certs_cm: std.map(kustomization, [holder.config_map_argocd_tls_certs_cm])[0],
    secret_argocd_secret: std.map(kustomization, [holder.secret_argocd_secret])[0],
  },
}
